...
def get_reward(config, net_shapes, params, seed_and_id):
    env = gym.make(config['game'], obs_type='ram')
...
class DistributedESAgent:
    ...
    def train(self, n_generations, log_data):
        ...
        with mp.Pool(processes=n_cores) as pool:
            for _ in tqdm(range(n_generations), desc="ES Distributed Assault"):
...
def run_es_distributed(log_data):
    print("\nRunning Distributed Evolutionary Strategies on Assault-v4 (RAM)...")
    GAME_CONFIG = {
        "game": "Assault-v4", "n_feature": 128, "n_action": 7, 
        "ep_max_step": 1000, "sigma": 0.05,
    }
    agent = DistributedESAgent(GAME_CONFIG, n_individuals=20, learning_rate=0.05)
    log_data = agent.train(n_generations=50, log_data=log_data)
    return log_data 